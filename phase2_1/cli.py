"""Phase 2.1 CLI: train SegFormer-B3 on Danish Golf Courses."""
...
@click.group()
@click.version_option(version="0.1.0", prog_name="phase2_1")
def cli():
    """Phase 2.1 – SegFormer semantic segmentation for golf courses."""
    pass
...
@click.option(
    ...
    default=Path(__file__).resolve().parents[1].parent / "phase2a" / "resources" / "archive.zip",  # course-builder repo
    help="Path to Danish Golf Courses archive.zip",
)
@click.option(
    "-o", "--output-dir",
    type=click.Path(path_type=Path),
    default=Path("phase2_1_output"),
