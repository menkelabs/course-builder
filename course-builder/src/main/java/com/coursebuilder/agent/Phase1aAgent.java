...
import com.coursebuilder.tool.golfcourse.Phase1aMcpTool;
...
/**
 * Agent specialized in Phase1a - Automated Satellite Tracing.
 * 
 * This agent handles:
 ...
 */
@Component
public class Phase1aAgent extends AbstractAgent {
    ...
    public String getDescription() {
        return "Specialized in Phase1a automated satellite tracing using SAM (Segment Anything Model). " +
    ...
    public AgentResponse process(AgentContext context, String userMessage) {
        log.info("Phase1aAgent processing: {}", userMessage);
        ...
        // Default: show Phase1a capabilities
        return AgentResponse.withFollowUp(
            "# Phase1a - Automated Satellite Tracing\n\n" +
            "I can help with SAM-based course feature extraction:\n\n" +
            "- **Run full pipeline**: `phase1a run satellite.png`\n" +
            ...
            "Tell me what Phase1a operation you need"
        );
    }
    ...
    private AgentResponse handleRunPipeline(AgentContext context, String userMessage) {
        ...
        return AgentResponse.withTools(
            "Running complete Phase1a pipeline...\n\n" + result.message() + "\n\n" +
    ...
    private AgentResponse handleGenerateMasks(AgentContext context, String userMessage) {
        String courseId = context.getCurrentCourseId();
        if (courseId == null) courseId = "demo-course";
        
        ToolResult result = executeTool("phase1a_mcp", Map.of(
            "operation", "phase1a_generate_masks",
            "parameters", Map.of(
                "courseId", courseId,
                "satelliteImage", extractPath(userMessage, "/input/satellite.png"),
                "checkpoint", "checkpoints/sam_vit_h_4b8939.pth"
        ...
        return AgentResponse.withTools(
            ...
            List.of(AgentResponse.ToolInvocation.of("phase1a_generate_masks", Map.of(), result.data()))
        );
    }
    ...
    private AgentResponse handleClassify(AgentContext context, String userMessage) {
        ...
        ToolResult result = executeTool("phase1a_mcp", Map.of(
            "operation", "phase1a_classify",
            "parameters", Map.of(
                "courseId", courseId,
                "masksDir", "/output/" + courseId + "/phase1a/masks/"
            )
        ), context);
        ...
        return AgentResponse.withTools(
            ...
            List.of(AgentResponse.ToolInvocation.of("phase1a_classify", Map.of(), result.data()))
        );
    }
    ...
    private AgentResponse handleGenerateSvg(AgentContext context, String userMessage) {
        ...
        ToolResult result = executeTool("phase1a_mcp", Map.of(
            "operation", "phase1a_generate_svg",
            "parameters", Map.of(
                "courseId", courseId,
                "includeHole98", true,
                "includeHole99", true
        ...
        return AgentResponse.withTools(
            ...
            List.of(AgentResponse.ToolInvocation.of("phase1a_generate_svg", Map.of(), result.data()))
        );
    }
    ...
    private AgentResponse handleValidate(AgentContext context, String userMessage) {
        ...
        return AgentResponse.withTools(
            "Phase1a Output Validation\n\n" + validationStatus + 
            "\n\nFiles checked: " + result.data().get("filesChecked"),
            List.of(AgentResponse.ToolInvocation.of("phase1a_validate", Map.of(), result.data()))
        );
    }
