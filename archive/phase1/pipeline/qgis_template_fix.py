"""
Additional fixes for QGIS template to ensure layers are visible and map is positioned.

This creates a Python script that QGIS can run on startup to fix layer visibility
and map positioning if the project file doesn't handle it correctly.
"""
...
def create_map_positioning_script(
    output_path: Path,
    initial_location: Optional[Dict[str, float]] = None
) -> Path:
    """
    Create a Python script that QGIS can run to position the map and ensure layers are visible.
    ...
    if initial_location:
        ...
        # Calculate scale value for the script (calculate here, use in f-string)
        meters_per_pixel = 156543.03392 / (2 ** zoom)
        target_scale_value = int(meters_per_pixel * 1000)
        
        # Calculate extent
        zoom_factor = 0.01 / (2 ** (19 - zoom))
        buffer = zoom_factor * 5
        
        # Use f-string with double braces {{}} for literal braces in the generated Python code
        script_content = f'''"""
...
'''
        ...
        script_content = '''"""
QGIS Map Positioning Script
Run this in QGIS Python Console to ensure layers are visible.
"""
...
# Ensure Bing Aerial layer is visible
bing_layer = None
for layer in project.mapLayers().values():
    if "Bing" in layer.name() and "Aerial" in layer.name():
        bing_layer = layer
...
if bing_layer:
    bing_layer.setOpacity(1.0)
    bing_layer.triggerRepaint()
    print("✓ Bing Aerial layer made visible")
else:
    print("⚠ Bing Aerial layer not found")
...
print("Layer visibility check complete!")
'''
    
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(script_content)
    logger.info(f"✓ Created map positioning script: {output_path}")
    return output_path
