"""
Tests for the main Phase1A client.
"""
...
from phase1a.client import Phase1AClient, PipelineStage, PipelineState
...
class TestPhase1AClient:
    """Tests for Phase1AClient class."""
    ...
    def test_output_dir_created(self, temp_dir):
        config = Phase1AConfig(output_dir=temp_dir / "new_output")
        client = Phase1AClient(config)
    ...
    def test_reset(self, temp_dir):
        config = Phase1AConfig(output_dir=temp_dir)
        client = Phase1AClient(config)
...
class TestPhase1AClientPipelineStages:
    ...
    def test_generate_masks_requires_image(self):
        ...
        config = Phase1AConfig()  # No input image
        client = Phase1AClient(config)
...
class TestPhase1AClientIntegration:
    """Integration tests for Phase1AClient (without SAM)."""
